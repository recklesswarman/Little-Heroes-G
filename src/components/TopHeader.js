...
export function renderTopHeader() {
  ...
  return `
    <header class="bg-surface/95 backdrop-blur-md font-headline docked sticky top-0 border-b-4 border-surface-container-highest shadow-chunky-sm flex justify-between items-center px-2 sm:px-4 md:px-8 h-16 sm:h-18 md:h-20 w-full z-40 max-w-full select-none overflow-hidden">
      <!-- Left: App Brand & Hero Avatar -->
      <div class="flex items-center gap-1.5 sm:gap-2.5 flex-shrink min-w-0">
        <button id="header-profile-btn" class="flex items-center gap-1.5 sm:gap-2 bg-surface-container hover:bg-surface-bright rounded-2xl p-1 sm:p-1.5 pr-2 sm:pr-3 border-2 border-surface-container-highest chunky-btn-sm transition-all group active:scale-95 min-h-[44px] flex-shrink min-w-0">
          ...
          <div class="flex flex-col text-left min-w-0">
            ...
            <span class="font-headline text-xs sm:text-sm md:text-base font-black text-secondary leading-tight truncate max-w-[55px] sm:max-w-[100px] md:max-w-none">${hero.name}</span>
          </div>
        ...
        <button id="header-hero-hq-btn" title="Hero HQ Bedroom & Secret Base" aria-label="Hero HQ" class="bg-surface-container hover:bg-surface-bright text-primary rounded-2xl px-2.5 py-1.5 sm:px-3 sm:py-2 border-2 border-emerald-400/40 chunky-btn-sm transition-all flex items-center gap-1 active:scale-95 min-h-[44px] flex-shrink-0">
      ...
      <div class="flex items-center gap-1 sm:gap-1.5 md:gap-2.5 flex-shrink-0">
        <!-- Points (⭐) - Real-Life Reward Currency -->
        <div title="Gold Points: Used for Real-Life Rewards (Needs Parent Approval)" class="flex items-center bg-surface-container-high rounded-full px-2 py-1 sm:px-2.5 sm:py-1.5 border-2 border-tertiary-container/70 gap-1 shadow-sm flex-shrink-0 min-h-[36px]">
          <span class="material-symbols-outlined text-tertiary text-sm sm:text-base md:text-lg" style="font-variation-settings: 'FILL' 1;">star</span>
        ...
        <div title="Habit Coins: Earned from all in-app quests & games" class="flex items-center bg-surface-container-high rounded-full px-2 py-1 sm:px-2.5 sm:py-1.5 border-2 border-secondary-container/70 gap-1 shadow-sm flex-shrink-0 min-h-[36px]">
          <span class="material-symbols-outlined text-secondary animate-coin text-sm sm:text-base md:text-lg" style="font-variation-settings: 'FILL' 1;">monetization_on</span>
        ...
        <button id="header-sync-btn" title="Household Device Link" aria-label="Sync Household" class="bg-surface-container text-on-surface-variant rounded-xl w-11 h-11 min-w-[44px] min-h-[44px] chunky-btn-sm border-surface-container-low hover:bg-surface-bright active:scale-95 flex items-center justify-center transition-all flex-shrink-0">
        ...
        <!-- Sound Toggle Button -->
        <button id="header-sound-btn" title="Toggle Sound FX" aria-label="Toggle Sound" class="bg-surface-container text-on-surface-variant rounded-xl w-11 h-11 min-w-[44px] min-h-[44px] chunky-btn-sm border-surface-container-low hover:bg-surface-bright active:scale-95 flex items-center justify-center transition-all flex-shrink-0">
